...
class GeoLocationRestMapper {
    ...
    fun fromCreateGeoLocationRequestToModel(
        ...
        return when (type) {
            ...
            GeoLocationType.REGION -> RegionModel(id = GeoLocationId(0), name = createRequest.name, boundaryRepresentation = boundary, continent = parent as ContinentModel)
            GeoLocationType.COUNTRY -> CountryModel(id = GeoLocationId(0), name = createRequest.name, boundaryRepresentation = boundary, region = parent as RegionModel)
            GeoLocationType.PROVINCE -> ProvinceModel(id = GeoLocationId(0), name = createRequest.name, boundaryRepresentation = boundary, country = parent as CountryModel)
            GeoLocationType.CITY -> CityModel(id = GeoLocationId(0), name = createRequest.name, boundaryRepresentation = boundary, province = parent as ProvinceModel)
            GeoLocationType.DISTRICT -> DistrictModel(id = GeoLocationId(0), name = createRequest.name, boundaryRepresentation = boundary, city = parent as CityModel)
        }
    }
